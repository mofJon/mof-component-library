    ...
    <ModuleBase data={data}>
      ...
      >
        {data.columns.map((col, index) => (
          <div
            className={`column-wrapper font-primary text-black flex flex-col ${colomnWidth} h-full`}
            key={index}
          >
            <div className="heading flex flex-col gap-4">
              {col.preHeading && (
                <PreHeading className="text-grey3 font-semibold">
                  {col.preHeading}
                </PreHeading>
              )}
              {col.headingTitle && (
                <HeadingTag
                  ...
                  className={
                    col.headingTitle.htag !== ""
                      ? `text-${col.headingTitle.htag}`
                      : "text-h2"
                  }
                />
              )}
