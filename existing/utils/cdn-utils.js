...
const subscriptionId = process.env.AZURE_SUBSCRIPTION_ID || 'a8d474fa-c550-4ef4-8d3d-5b56f9b837e5';
...
const loadAssetsToCdnCache = async () => {
  if ((process.env.NODE_ENV === 'production' || DEV_MODE) && endpointName) {
    ...
    try {
      const homepapeRes = await fetch(`${process.env.RESOURCE_PATH || 'https://qa.matterofform.com/'}`);
      ...
      if (homepapeRes && homepapeRes.ok) {
        ...
        console.error(`[CU] Error fetching ${process.env.API_URL || 'https://qa-bb-api.matterofform.com/'}`);
      }
    } catch (err) {
    ...
    try {
      const folders = [{ path: '/_next/static/media/', dir: './.next/static/media/' }];
    ...
    console.info('[CU] Skipping assets cache build, not production environment.');
  }
};
  ...
  if (process.env.NODE_ENV === 'production' || DEV_MODE) {
    ...
    for await (const link of links) {
      try {
        const url = new URL(
          link,
          !link.startsWith('http:') && !link.startsWith('https:') ? 'http://fake-base.com' : undefined,
        );
        ...
        const htmlRes = await fetch(process.env.RESOURCE_PATH + pathname + url.search + url.hash, {
          headers: { 'x-cdn-load': '1', 'x-cdn-load-refresh': '1' },
        });
          ...
          if (sectionMatch) {
            ...
            srcsets.forEach((srcset) => {
              ...
              srcsetArray.forEach((image) => {
                if (image.startsWith('/media') && image.includes('format=webp')) {
                  imageSet.add(image);
                }
              });
            });
    ...
    for await (const path of urlSet) {
      ...
      if (urlChunkSet.size === URL_CHUNK_SIZE || counter === links.length) {
        ...
        console.info(
          `[CU] CDN HTML load ${
            counter !== links.length ? Math.round((counter * 100) / links.length) : 100
          }% compleate`,
        );
      }
    }
    ...
    if (imageList.length > 0) {
      ...
      for await (const image of imageList) {
        ...
        if (imageChunkSet.size === IMAGE_CHUNK_SIZE || counter === imageList.length) {
          try {
          ...
          console.info(
            `[CU] CDN image load ${
              counter !== imageList.length ? Math.round((counter * 100) / imageList.length) : 100
            }% compleate`,
          );
        }
      }
...
const purgeCdnCache = async (contentPaths = ['/*']) => {
  if ((process.env.NODE_ENV === 'production' || DEV_MODE) && endpointName) {
    ...
    try {
      ...
      await cdnClient.endpoints.beginPurgeContentAndWait(resourceGroupName, profileName, endpointName, {
        contentPaths,
      });
...
async function loadPaths(contentPaths) {
  if ((contentPaths && contentPaths.length > 0 && process.env.NODE_ENV === 'production') || DEV_MODE) {
    var cdnClient = getCdnClient();
    const urlSet = [];
